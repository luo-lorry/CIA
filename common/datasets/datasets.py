...
def generate_synthetic_data(n_samples):
    # Generate X
    X = np.random.choice([0, 1], size=n_samples, p=[0.8, 0.2])

    # Generate Y
    y = np.zeros(n_samples)

    # For X=0, Y follows uniform[0,1]
    y[X == 0] = np.random.uniform(0, 1, size=np.sum(X == 0))

    # For X=1, Y follows uniform[0,10]
    y[X == 1] = np.random.uniform(0, 10, size=np.sum(X == 1))
...
def generate_heteroscedastic_data(n_samples):
    ...
    mean = X ** 2 - X
    std_dev = np.sqrt(X ** 2)  # standard deviation is sqrt of variance

    # Generate Y: normal distribution with calculated mean and standard deviation
    y = np.random.normal(loc=mean.flatten(), scale=std_dev.flatten())

    return X, y


def generate_data(num_samples):
    # Sample x from uniform distribution (0, 1)
    x = np.random.uniform(0, 1, num_samples)

    # Generate y using the triangle distribution with parameters (0, x, x)
    y = np.random.triangular(0, x, x)

    return x.reshape(-1, 1), y.flatten()


def GetDataset(name, base_path, symmetry=3, n_data=10000):
    """ Load a dataset
	...
	"""
    base_path = "datasets/" # "C:/Users/lorry/OneDrive - City University of Hong Kong/Documents/github repos/20231229-er_cp/conformal_regression/examples/datasets/" # os.path.dirname(os.path.abspath(__file__)) + r'\\' # pathlib.Path().resolve() # pathlib.Path(__file__).parent.resolve()
    print(base_path)
    if name=="synthetic":
