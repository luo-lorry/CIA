...
class DirectedEdgeDecoder(torch.nn.Module):
    """
    Edge Decoder module to infer the class logits for edge labels.

    Args:
    in_channels (int): The number of input channels.
    hidden_channels (int): The number of hidden channels.
    out_channels (int): The number of output channels (number of classes).
    """

    def __init__(self, in_channels, hidden_channels=8, out_channels=3):
        super().__init__()
    ...
    def forward(self, z1, z2, edge_index):
        ...
        Args:
        z1 (torch.Tensor): Node embeddings for the source nodes.
        z2 (torch.Tensor): Node embeddings for the target nodes.
        edge_index (torch.Tensor): Edge index tensor.

        Returns:
        torch.Tensor: Predicted class logits for edge labels.
        """
        row, col = edge_index
...
class DirectedEdgeDecoderRegression(torch.nn.Module):
     """
     Edge Decoder module to infer the predictions. 
